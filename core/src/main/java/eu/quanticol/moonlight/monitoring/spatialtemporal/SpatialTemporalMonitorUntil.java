 ...
 *
 */
public class SpatialTemporalMonitorUntil<E,S,T> implements SpatialTemporalMonitor<E, S, T> {
	...
	public SpatialTemporalMonitorUntil(SpatialTemporalMonitor<E, S, T> m1, Interval interval,
	...
	public SpatialTemporalSignal<T> monitor(LocationService<E> locationService, SpatialTemporalSignal<S> signal) {
		return SpatialTemporalSignal.applyToSignal(m1.monitor(locationService, signal), (s1, s2) -> TemporalMonitorUntil.computeUntil(domain, s1, interval, s2), m2.monitor(locationService, signal));
	}
