...
class Network():
    ...
    def __init__(self,node_number, state_model, main_degree_distribution, event_degree_distribution, event_probability, infection_probability_distribution):
...
class SimulationOutput():
     ...
     def __init__(self, network, trajectories, final_size, steps, runs, max_steps, new_infection_explosion_threshold = 10):
         ...
         self.mask_only_explosive = np.amax(self.trajectories["new_infections"],axis = 1) > self.new_infection_explosion_threshold
     ...
     def plot_mean_trajectory(self,show=True,file=None, confidence_level = 1.96, title="epidemic evolution", only_explosive = True):
        for s in self.states:
            if only_explosive:
            ...
            plt.fill_between(self.trajectories["days"], means - confidence_level*stds, means + confidence_level*stds, alpha=0.2)
