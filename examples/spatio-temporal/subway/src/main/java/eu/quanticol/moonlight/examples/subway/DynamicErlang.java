...
import static eu.quanticol.moonlight.examples.subway.DynamicErlangSignal.*;
 ...
 *
 */
public class DynamicErlang {
    private static final Logger LOG = Logger.getLogger(DynamicErlang.class.getName());
   ...
   private static final Collection<MultiValuedTrace> data =
    ...
    public static void realData() {
        ...
        LocationService<Double, Double> locService =
                    createOrientedLocSvc(device.getFirst(), device.getSecond());

        MultiRawTrajectoryExtractor trajectory =
                new MultiRawTrajectoryExtractor(network.size(), processor);

        smc(phi1(SATISFACTION), "real_s_p1", data, locService);
        smc(phi1(ROBUSTNESS), "real_r_p1", data, locService);
        smc(phi2(SATISFACTION), "real_s_p2", data, locService);
        smc(phi2(ROBUSTNESS), "real_r_p2", data, locService);
        smc(phi3(SATISFACTION), "real_s_p3", data, locService);
        smc(phi3(ROBUSTNESS), "real_s_p3", data, locService);
        smc(phi4(SATISFACTION), "real_s_p4", data, locService);
        smc(phi4(ROBUSTNESS), "real_r_p4", data, locService);
    }
    ...
    private static String outputFile(String path, String ext1, String ext2) {
        ...
        String newpath = DynamicErlang.class.getResource("/").getPath() + trace + path;
    ...
    private static <D> SpatialTemporalMonitor<Double, List<Comparable<?>>, D> phi2(SignalDomain<D> d) {
        return impliesMonitor(
                    ...
                    d,
                    somewhereMonitor(isNotCrowded(d), Grid.distance(1,2)
                            , d)
                    )
        ;
    }
