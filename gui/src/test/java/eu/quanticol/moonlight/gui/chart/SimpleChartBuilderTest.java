...
class SimpleChartBuilderTest {
    ...
    @Test
    void createSeriesForConstantChartTest() throws IOException {
        File csv = new File((Objects.requireNonNull(ClassLoader.getSystemClassLoader().getResource("file.csv"))).getFile());
        ...
        attributes.add("v");
        simpleChartBuilder.getGraphController().setColumnsAttributes(attributes);
        ArrayList<Series<Number, Number>> series = simpleChartBuilder.createSeriesForConstantChart(csv,5);
        assertEquals(series.size(), 10);
        assertEquals(series.get(0).getData().get(0).toString(), new Data<>(0.0, 0.0).toString());
        assertEquals(series.get(0).getData().get(2).toString(), new Data<>(1.0, -4.538651278767508).toString());
        assertEquals(series.get(0).getData().get(4).toString(), new Data<>(2.0, -8.43293853755839).toString());
    }
    ...
    void getSeriesFromNodesTest() {
        ...
        SimpleChartBuilder simpleChartBuilder = new SimpleChartBuilder();
        List<Series<Number, Number>> series = simpleChartBuilder.getSeriesFromNodes(list,4);
    ...
    void getSeriesFromStaticGraphTest() {
        ...
        attributes.add("v");
        simpleChartBuilder.getGraphController().setColumnsAttributes(attributes);
        ArrayList<Series<Number, Number>> series = simpleChartBuilder.getSeriesFromStaticGraph(line, list, false,5);
    ...
    void addLineDataTest() {
        ...
        String line = "0, 155.05290550508354, 197.4287636921495, 5.696894467052598, 0.9011928036062729, 0, 138.7384637743081, 97.47360568313172, 3.9675733703908946, 0.255938829010682, 25, 92.90203975390901, 188.15696964044184, 6.114450372652869, 0.8553343447074815, 10, 88.79064023230296, 117.00681495028866, 1.0626328870598654, 0.4560818434879118, 0, 4.748694971371492, 30.985655177864114, 0.7224531443252975, 0.861984983062122, 0\n";
        SimpleChartBuilder simpleChartBuilder = new SimpleChartBuilder();
        list = simpleChartBuilder.getSeriesFromStaticGraph(line, list, false,5);
        ...
        simpleChartBuilder.addLineData(list, attributes,5);
