...
class MultiTrajectoryExtractorTest {

    private final String[] inputHeader = {"V0", "V1", "V2"};

    @Test
    void trivialInput() {
        MultiTrajectoryExtractor t = new MultiTrajectoryExtractor(1);
    ...
    void simpleGoodFileProcessing() {
        ...
        MultiTrajectoryExtractor t = new MultiTrajectoryExtractor(2);
        ...
        HashBiMap<Integer, Integer, Double> data = t.result().iterator().next();

        // We expect a space size of 2
        assertEquals(2, data.keySet1().size());

        // We expect a time span of 3
        assertEquals(3, data.keySet2().size());
