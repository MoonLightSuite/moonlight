...
import io.github.moonlightsuite.moonlight.online.algorithms.TemporalOp;
        ...
        implements OnlineMonitor<Double, V, Box<R>>
{
            ...
            Update<Double, V> signalUpdate)
    {
        ...
        for(TimeChain<Double, Box<R>> argU: argUpdates) {
            updates.addAll(TemporalOp.slidingWindow(s, argU,
                                                             horizon, op));
        }
            ...
            TimeChain<Double, V> updates)
    {
        ...
        for(TimeChain<Double, Box<R>> argU: argUpdates) {
            output.addAll(TemporalOp.slidingWindow(s, argU,
                    horizon, op));
        }
